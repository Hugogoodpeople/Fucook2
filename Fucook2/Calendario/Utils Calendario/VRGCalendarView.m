...
-(void)showNextMonth {
    ...
    UIView *animationHolder = [[UIView alloc] initWithFrame:CGRectMake(0, kVRGCalendarViewTopBarHeight, self.frame.size.width, targetSize-kVRGCalendarViewTopBarHeight)];
...
-(void)showPreviousMonth {
    ...
    float targetSize = fmaxf(oldSize, self.calendarHeight);
    UIView *animationHolder = [[UIView alloc] initWithFrame:CGRectMake(0, kVRGCalendarViewTopBarHeight, [UIScreen mainScreen].bounds.size.width-20, targetSize-kVRGCalendarViewTopBarHeight)];
...
{       
    ...
    if (touchPoint.y > kVRGCalendarViewTopBarHeight) {
        ...
        int column = floorf(xLocation/((([UIScreen mainScreen].bounds.size.width -36 )/7)+2));
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i =0; i<[weekdays count]; i++) {
        ...
        [weekdayValue drawInRect:CGRectMake(i*((([UIScreen mainScreen].bounds.size.width -36 )/7)+2), 40, (([UIScreen mainScreen].bounds.size.width -36 )/7)+2, 20) withFont:font lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
    }
    ...
    CGContextAddLineToPoint(context, [UIScreen mainScreen].bounds.size.width-20, kVRGCalendarViewTopBarHeight+1);
    for (int i = 1; i<7; i++) {
        CGContextMoveToPoint(context, i*((([UIScreen mainScreen].bounds.size.width -36 )/7)+1)+i*1-1, kVRGCalendarViewTopBarHeight);
        CGContextAddLineToPoint(context, i*((([UIScreen mainScreen].bounds.size.width -36 )/7)+1)+i*1-1, kVRGCalendarViewTopBarHeight+gridHeight);
        
        if (i>numRows-1) continue;
        //rows
        CGContextMoveToPoint(context, 0, kVRGCalendarViewTopBarHeight+i*(kVRGCalendarViewDayHeight+1)+i*1+1);
        CGContextAddLineToPoint(context, [UIScreen mainScreen].bounds.size.width-20, kVRGCalendarViewTopBarHeight+i*(kVRGCalendarViewDayHeight+1)+i*1+1);
    }
    ...
    CGContextAddLineToPoint(context, [UIScreen mainScreen].bounds.size.width-20, kVRGCalendarViewTopBarHeight);
    for (int i = 1; i<7; i++) {
        //columns
        CGContextMoveToPoint(context, i*((([UIScreen mainScreen].bounds.size.width -36 )/7)+1)+i*1, kVRGCalendarViewTopBarHeight);
        CGContextAddLineToPoint(context, i*((([UIScreen mainScreen].bounds.size.width -36 )/7)+1)+i*1, kVRGCalendarViewTopBarHeight+gridHeight);
        
        if (i>numRows-1) continue;
        //rows
        CGContextMoveToPoint(context, 0, kVRGCalendarViewTopBarHeight+i*(kVRGCalendarViewDayHeight+1)+i*1);
        CGContextAddLineToPoint(context, [UIScreen mainScreen].bounds.size.width-20, kVRGCalendarViewTopBarHeight+i*(kVRGCalendarViewDayHeight+1)+i*1);
    }
    CGContextMoveToPoint(context, 0, gridHeight+kVRGCalendarViewTopBarHeight);
    CGContextAddLineToPoint(context, [UIScreen mainScreen].bounds.size.width-20, gridHeight+kVRGCalendarViewTopBarHeight);
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        int targetRow = i/7;
        int targetX = targetColumn * ((([UIScreen mainScreen].bounds.size.width -36 )/7)+2);
        ...
        [date drawInRect:CGRectMake(targetX+2, targetY+10, (([UIScreen mainScreen].bounds.size.width -36 )/7), kVRGCalendarViewDayHeight) withFont:[UIFont fontWithName:@"HelveticaNeue-Bold" size:17] lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
    }
...
-(UIImage *)drawCurrentState {
    ...
    UIGraphicsBeginImageContext(CGSizeMake([UIScreen mainScreen].bounds.size.width-20, targetHeight-kVRGCalendarViewTopBarHeight));
...
-(id)init {
    ...
    if (self) {
        ...
        isAnimating=NO;
        self.labelCurrentMonth = [[UILabel alloc] initWithFrame:CGRectMake(34, 0, [UIScreen mainScreen].bounds.size.width-20-68, 40)];
